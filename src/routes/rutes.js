...
class ClassRouter {
    ...
    getRouter(){
        return this.router //vienen todas mis rutas
    }

    init(){}

    // let saludo(...param){} // param ['sofi', 'emiliano']
    // saludo('sofi', 'emiliano')


    // (req,res,next,etc)
    applyCallbacks(callbacks){ // params req, res, next, etc
        return callbacks.map(callback => async (...params)=>{ //(...params)asi hago para guardar en un array
            try {
    ...
    handlePolicies = policies =>  (req, res, next) =>{
        ...
        const token  = authHeaders.split(' ')[1] // que separe y me de el token beare tasdfhashdkfa 
